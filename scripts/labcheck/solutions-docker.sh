...
sol_ch_dock_lab3_3='curl -s http://localhost:5000/ > /root/app_response.txt'
...
sol_ch_dock_lab4_3='curl -s http://localhost:8080/ > /root/go_response.txt'
...
sol_ch_dock_lab4_6='{ docker ps -a --format "{{.Names}}"; docker images --format "{{.Repository}}:{{.Tag}}"; } > /root/lab4_result.txt'
...
sol_ch_dock_lab7_2='docker images registry.company.local/devops/mynginx > /root/images.txt'
