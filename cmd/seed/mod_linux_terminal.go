...
func mod_linux_terminal() M {
	return M{
		...
		Title:       "Linux: практикум в терминале (автопроверка)",
		Track:       "devops",
		Difficulty:  "beginner",
...
<p>Подсказка: <code>grep ERROR /root/log.txt &gt; /root/errors.txt</code>. Посмотреть исходник — <code>cat /root/log.txt</code>.</p>`,
						...
						CheckScript: `if [ -f /root/errors.txt ] && [ "$(grep -c ERROR /root/errors.txt)" = 2 ] && [ "$(wc -l < /root/errors.txt)" = 2 ]; then echo "✓ В errors.txt ровно 2 строки с ERROR"; else echo "✗ Нужен файл ~/errors.txt из 2 строк, обе с ERROR. Сейчас строк с ERROR: $(grep -c ERROR /root/errors.txt 2>/dev/null), всего строк: $(wc -l < /root/errors.txt 2>/dev/null)"; exit 1; fi`,
...
<p>Подсказка: <code>find /root/proj -name "*.md" &gt; /root/md.txt</code>.</p>`,
						...
						CheckScript: `if [ -f /root/md.txt ] && [ "$(grep -c '\.md' /root/md.txt)" = 2 ]; then echo "✓ В md.txt 2 строки с .md"; else echo "✗ Нужен файл ~/md.txt с 2 строками, содержащими .md. Сейчас совпадений: $(grep -c '\.md' /root/md.txt 2>/dev/null)"; exit 1; fi`,
...
<p>Проверь результат: <code>ls -l /root/run.sh</code> — в правах должна появиться <code>x</code>.</p>`,
						...
						CheckScript: `if [ -f /root/run.sh ] && [ -x /root/run.sh ]; then echo "✓ run.sh существует и исполняемый"; else echo "✗ не выполнено: ~/run.sh существует и имеет флаг исполнения"; exit 1; fi`,
...
<p>Подсказка: <code>chmod 600 /root/secret.txt</code>. Проверь: <code>ls -l /root/secret.txt</code>.</p>`,
						...
						CheckScript: `if [ "$(stat -c '%a' /root/secret.txt 2>/dev/null)" = 600 ]; then echo "✓ Права на secret.txt — 600"; else echo "✗ не выполнено: права на ~/secret.txt — 600. Сейчас: $(stat -c '%a' /root/secret.txt 2>/dev/null)"; exit 1; fi`,
...
<p>Последняя строка файла должна быть ровно <code>END</code>.</p>`,
						...
						CheckScript: `if [ -f /root/etc.txt ] && [ "$(tail -n1 /root/etc.txt)" = "END" ] && [ "$(wc -l < /root/etc.txt)" -gt 1 ]; then echo "✓ etc.txt заканчивается строкой END"; else echo "✗ Нужен файл ~/etc.txt более чем из одной строки, последняя — END. Проверь: tail -n1 ~/etc.txt"; exit 1; fi`,
...
<p>Подсказка: <code>ls /root/cfg | grep '\.conf$' | wc -l &gt; /root/n.txt</code>.</p>`,
						...
						CheckScript: `if [ "$(tr -d ' \n' < /root/n.txt 2>/dev/null)" = 2 ]; then echo "✓ В n.txt получилось число 2"; else echo "✗ В файле ~/n.txt должно получиться число 2. Сейчас: $(cat /root/n.txt 2>/dev/null)"; exit 1; fi`,
...
<p>Подсказка: <code>sort /root/names.txt | uniq &gt; /root/unique.txt</code>.</p>`,
						...
						CheckScript: `if [ -f /root/unique.txt ] && [ "$(wc -l < /root/unique.txt)" = 3 ] && [ "$(head -1 /root/unique.txt)" = alice ]; then echo "✓ unique.txt: 3 строки, первая — alice"; else echo "✗ Нужен ~/unique.txt из 3 строк, первая — alice (подсказка: sort | uniq). Сейчас строк: $(wc -l < /root/unique.txt 2>/dev/null), первая: $(head -1 /root/unique.txt 2>/dev/null)"; exit 1; fi`,
...
<p>Подсказка: <code>cut -d: -f1 /root/users.csv &gt; /root/just_names.txt</code>.</p>`,
						...
						CheckScript: `if [ -f /root/just_names.txt ] && [ "$(wc -l < /root/just_names.txt)" = 3 ] && grep -qx alice /root/just_names.txt && ! grep -q ':' /root/just_names.txt; then echo "✓ just_names.txt: 3 имени без двоеточий"; else echo "✗ Нужен ~/just_names.txt из 3 строк-имён (есть alice), без символа ':' (подсказка: cut -d: -f1). Проверь: cat ~/just_names.txt"; exit 1; fi`,
...
<p>Подсказка: <code>echo $HOME &gt; /root/home.txt</code>.</p>`,
						...
						CheckScript: `if [ "$(cat /root/home.txt 2>/dev/null)" = "/root" ]; then echo "✓ home.txt содержит /root"; else echo "✗ В ~/home.txt запиши значение переменной \$HOME (echo \$HOME > home.txt). Сейчас: $(cat /root/home.txt 2>/dev/null)"; exit 1; fi`,
...
<p>Подсказка: <code>GREETING=hello; echo $GREETING &gt; /root/greet.txt</code> (в одной строке, потому что переменная живёт только в пределах команды).</p>`,
						...
						CheckScript: `if [ "$(cat /root/greet.txt 2>/dev/null)" = "hello" ]; then echo "✓ greet.txt содержит hello"; else echo "✗ В ~/greet.txt должно быть слово hello. Сейчас: $(cat /root/greet.txt 2>/dev/null)"; exit 1; fi`,
...
<p>Задание зачтётся, когда процесс <code>sleep 100000</code> перестанет существовать.</p>`,
						...
						CheckScript: `if [ -z "$(pgrep -f 'sleep 10000[0]')" ]; then echo "✓ Фоновый процесс завершён"; else echo "✗ не выполнено: фоновый процесс sleep завершён"; exit 1; fi`,
...
<p>Подсказка: <code>ps -p 1 -o comm= &gt; /root/pid1.txt</code>.</p>`,
						...
						CheckScript: `if [ -s /root/pid1.txt ] && grep -qi sleep /root/pid1.txt; then echo "✓ pid1.txt содержит строку процесса sleep"; else echo "✗ не выполнено: в ~/pid1.txt лежит строка с процессом sleep"; exit 1; fi`,
...
<p>Подсказка: <code>echo "$(whoami):$(id -u)" &gt; /root/me.txt</code>. В песочнице ты root, так что ждём <code>root:0</code>.</p>`,
						...
						CheckScript: `if [ "$(cat /root/me.txt 2>/dev/null)" = "root:0" ]; then echo "✓ me.txt содержит root:0"; else echo "✗ В ~/me.txt запиши 'имя:uid' пользователя — root:0 (подсказка: id -un и id -u). Сейчас: $(cat /root/me.txt 2>/dev/null)"; exit 1; fi`,
...
<p>Проверь результат: <code>id alice</code> или <code>grep alice /etc/passwd</code> — должна появиться запись.</p>`,
						...
						CheckScript: `if id alice >/dev/null 2>&1 && grep -q '^alice:' /etc/passwd; then echo "✓ Пользователь alice создан"; else echo "✗ Создай пользователя alice (useradd alice). Проверь: id alice"; exit 1; fi`,
...
<p>Подсказка: <code>chgrp daemon /root/report.txt</code> (или <code>chown :daemon /root/report.txt</code>). Проверь: <code>ls -l /root/report.txt</code>.</p>`,
						...
						CheckScript: `if [ "$(stat -c '%G' /root/report.txt 2>/dev/null)" = daemon ]; then echo "✓ Группа report.txt — daemon"; else echo "✗ Смени группу ~/report.txt на daemon (chgrp daemon report.txt). Сейчас группа: $(stat -c '%G' /root/report.txt 2>/dev/null)"; exit 1; fi`,
