...
<p>Проверь результат: <code>ls -l /root/run.sh</code> — в правах должна появиться <code>x</code>.</p>`,
						...
						CheckScript: `if [ -f /root/run.sh ] && [ -x /root/run.sh ]; then echo "✓ run.sh существует и исполняемый"; else echo "✗ Нужен исполняемый файл ~/run.sh (chmod +x run.sh). Проверь: ls -l ~/run.sh"; exit 1; fi`,
...
<p>Подсказка: <code>chmod 600 /root/secret.txt</code>. Проверь: <code>ls -l /root/secret.txt</code>.</p>`,
						...
						CheckScript: `if [ "$(stat -c '%a' /root/secret.txt 2>/dev/null)" = 600 ]; then echo "✓ Права на secret.txt — 600"; else echo "✗ Установи права 600 на ~/secret.txt (chmod 600 secret.txt). Сейчас: $(stat -c '%a' /root/secret.txt 2>/dev/null)"; exit 1; fi`,
...
<p>Подсказка: <code>ps -p 1 -o comm= &gt; /root/pid1.txt</code>.</p>`,
						...
						CheckScript: `if [ -s /root/pid1.txt ] && grep -qi sleep /root/pid1.txt; then echo "✓ pid1.txt содержит строку процесса sleep"; else echo "✗ Запиши в ~/pid1.txt строку процесса sleep (например: ps aux | grep sleep > pid1.txt)"; exit 1; fi`,
