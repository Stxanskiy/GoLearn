...
---</pre>`,
  ...
  script:
    ...
    name: production</pre>`,
		...
		Checks: map[int]string{
			1: check(`git -C /root/gitlab-ci-lab log --oneline -- README.md | wc -l | awk '$1>1{exit 0} {exit 1}'`,
  ...
  script:
    - cat /etc/alpine-release</pre>`,
		...
		Checks: map[int]string{
			...
			2: check(`git -C /root/gitlab-ci-lab log --oneline -- .gitlab-ci.yml | wc -l | awk '$1>1{exit 0} {exit 1}'`,
