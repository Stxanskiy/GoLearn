...
mkdir -p /root/project`,
		Checks: map[int]string{
			...
			5: check(`[ -f /root/status.txt ] && [ ! -s /root/status.txt ] && [ -z "$(git -C /root/project status --porcelain 2>/dev/null)" ]`,
				"короткий git status сохранён в /root/status.txt и пуст",
				"Сохрани короткий статус: git -C /root/project status --short > /root/status.txt (у чистого дерева файл пустой)"),
		},
	},
...
git -C /root/project commit -qam "main change"`,
		Checks: map[int]string{
			...
			5: check(`grep -q 'resolve conflict' /root/merge_log.txt 2>/dev/null && [ "$(git -C /root/project rev-list --parents -n1 HEAD | wc -w)" -ge 3 ]`,
				"граф истории с merge-коммитом сохранён в /root/merge_log.txt",
				"git -C /root/project log --oneline --graph -5 > /root/merge_log.txt"),
		},
	},
...
rm -rf /tmp/seedrepo`,
		Checks: map[int]string{
			...
			3: check(`grep -q 'feature' /root/bare_log.txt 2>/dev/null && git --git-dir=/srv/git/project.git show --name-only --pretty=format: HEAD 2>/dev/null | grep -q 'feature.txt'`,
				"лог bare-репозитория с feature.txt сохранён в /root/bare_log.txt",
				"git --git-dir=/srv/git/project.git log --oneline --stat -1 > /root/bare_log.txt"),
			4: check(`grep -q 'remotes/origin' /root/branches.txt 2>/dev/null`,
				"список веток с remotes/origin сохранён в /root/branches.txt",
				"git -C /root/project fetch origin && git -C /root/project branch -a > /root/branches.txt"),
...
printf 'feature v2 (не закоммичено)\n' >> /root/project/feature.txt`,
		Checks: map[int]string{
			...
			5: check(`grep -qi 'revert' /root/revert_log.txt 2>/dev/null && grep -q 'bad commit' /root/revert_log.txt`,
				"история с revert и плохим коммитом сохранена в /root/revert_log.txt",
				"git -C /root/project log --oneline > /root/revert_log.txt — в файле должны быть обе записи"),
		},
	},
...
git -C /root/project add -A && git -C /root/project commit -qm "wip fix"`,
		Checks: map[int]string{
			...
			3: check(`[ -f /root/status.txt ] && [ ! -s /root/status.txt ] && [ -z "$(git -C /root/project status --porcelain 2>/dev/null)" ]`,
				"короткий git status сохранён в /root/status.txt и пуст",
				"git -C /root/project status --short > /root/status.txt (после rebase файл должен остаться пустым)"),
		},
	},
