...
func (s *ShellRunner) ensure(ctx context.Context, userID int, key, image, setup string) (string, error) {
	...
	out, _, err := s.run(ctx, fmt.Sprintf("docker inspect -f '{{.State.Running}} {{.State.StartedAt}}' %s 2>&1", c))
	if err != nil {
		return "", err
	}
	...
	if at, running := runningSince(out); running {
		s.touch(c)
		s.mu.Lock()
		if s.started[c].IsZero() {
			s.started[c] = at
		}
		s.mu.Unlock()
		return c, nil
...
// runningSince parses `docker inspect -f '{{.State.Running}} {{.State.StartedAt}}'`.
func runningSince(out string) (time.Time, bool) {
	fields := strings.Fields(out)
	if len(fields) == 0 || fields[0] != "true" {
		return time.Time{}, false
	}
	if len(fields) > 1 {
		if at, err := time.Parse(time.RFC3339Nano, fields[1]); err == nil && !at.IsZero() {
			return at, true
		}
	}
...
// Session returns lifetime info of the user's container session, if one is alive.
func (s *ShellRunner) Session(userID int, key string) (SessionInfo, bool) {
	if !s.Enabled() {
	...
	s.mu.Lock()
	defer s.mu.Unlock()
	started, ok := s.started[c]
	if !ok {
		return SessionInfo{}, false
	}
	exp := started.Add(shellSessionMax)
	if idle := s.sessions[c].Add(shellSessionTTL); idle.Before(exp) {
		exp = idle
	}
	return SessionInfo{Started: started, ExpiresAt: exp}, true
}

// Touch marks the user's session as active (e.g. on terminal keystrokes).
func (s *ShellRunner) Touch(userID int, key string) {
	if s.Enabled() {
		s.touch(sessionName(userID, key))
	}
}

// HasSession reports whether a container for this user+lesson is currently alive,
// so the frontend can auto-reconnect after a page reload.
func (s *ShellRunner) HasSession(userID int, key string) bool {
	if !s.Enabled() {
		return false
	}
	s.mu.Lock()
	defer s.mu.Unlock()
	_, ok := s.started[sessionName(userID, key)]
	return ok
}
