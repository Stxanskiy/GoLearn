// Package simulators holds the built-in simulator scenarios and seeds them.
//
// The scenarios used to live in the HTML handler, which also rendered their
// pages. The pages are gone — the frontend owns them — but the data and the
// seeding are domain logic and belong on their own.
package simulators
...
// Store is the slice of the simulator repository this package needs.
...
// Ensure seeds the built-in scenarios once, so an admin can edit them afterwards.
// It does nothing when the table already holds rows: the stored copy is the one
// being edited, and overwriting it would discard those edits on every boot.
func Ensure(ctx context.Context, store Store) error {
	if store == nil {
		return nil
	}
	...
	for i, s := range scenarios() {
		...
		if err := store.Upsert(ctx, model.Simulator{
